...
# Helper: accumulate totals
add_sum() {
    TOTAL_SUM=$((TOTAL_SUM + $1))
    PASSED_SUM=$((PASSED_SUM + $2))
    FAILED_SUM=$((FAILED_SUM + $3))
}

while IFS= read -r file; do
    log_info "Processing $file"
    [[ -s "$file" ]] || { log_warn "File $file is empty or missing. Skipping."; continue; }

    # Read header directly (no subshell block)
    if read -r header < "$file"; then
        IFS="|" read -r OS MODE TOTAL PASSED FAILED <<< "$header"

        if [[ -n "$OS" && -n "$MODE" ]]; then
            rows+=("$OS|$MODE|$TOTAL|$PASSED|$FAILED")
            add_sum "$TOTAL" "$PASSED" "$FAILED"
